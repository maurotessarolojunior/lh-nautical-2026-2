...
Submissão/Dashboard/gerar_dados.py                      → script que gera os extratos a partir dos CSVs brutos
Submissão/Dashboard/README.md                           → este arquivo
Submissão/Dashboard/LH_Nautical_2026_2_Dashboard.pdf    → PDF final do painel, exportado do Looker Studio
Submissão/Apresentacao/LH_Nautical_2026_2_Apresentacao.pdf → apresentação de 10 slides que acompanha o painel
```

Essa separação é uma decisão consciente de organização, não uma arquitetura adicionada para parecer mais sofisticada: `data/processed/marts/dashboard/` guarda dados prontos para consumo; `Submissão/Dashboard/` guarda código, documentação e o produto visual. Inclusive depois do painel pronto, o PDF fica em `Submissão/Dashboard/`, não junto dos CSVs. Não existe camada `stage`/`intermediate` porque não há necessidade real dela neste desafio: ver "Linhagem dos dados" abaixo.
...
Não há junção com `order_items`, o que evita repetir pedido e inflar `valor_pedido`. Todos os status de pedido estão incluídos (premissa do desafio, sem filtro inventado); por isso o painel deve rotular os totais como "registrado", não como receita reconhecida.
...
O `product_id = 180` (produto de referência) nunca aparece no ranking: a exclusão é feita pelo próprio identificador, nunca por comparação de valor de similaridade.
...
Antes de gravar qualquer CSV, `gerar_dados.py` confere, com `assert` e `math.isclose(..., rel_tol=0.0, abs_tol=...)`. A tolerância é só absoluta (centésimos para valores monetários, `0,0001` para similaridade), nunca relativa, para que uma diferença de poucos centavos num total de R$ 1,4 bilhão não passe despercebida:
